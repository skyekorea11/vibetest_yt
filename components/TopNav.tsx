...
export default function TopNav({ nextRefresh, onManualRefresh }: TopNavProps) {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (diff <= 0) { setCountdown('갱신 예정'); return }
    ...
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-30">
      <div className="px-5">
        <div className="flex items-center justify-between h-14">
          ...
          <Link
            href="/"
            className="flex items-center gap-2 font-bold text-lg text-gray-900 hover:text-gray-700 transition-colors"
          >
            ...
            <div className="flex items-center gap-3">
              {countdown && (
                <div className="flex flex-col items-end" style={{ color: '#8b00ff' }}>
                  ...
                  <span className="text-sm font-bold tabular-nums leading-tight tracking-tight">
                    {countdown}
                  </span>
                </div>
              )}
